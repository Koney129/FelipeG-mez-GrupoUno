export const products = [
  {
      id: 1,
      name: 'Mouse Gamer Rgb',
      price: 35000,
<<<<<<< HEAD
      description: '¿Eres Gamer? Pues aqui tienes un articulo para jugar a los juegos que te gustan gracias a este mouse Gamer con entrada tipo Usb.',
      category: 'Accesorio para PC',
=======
      description: '¿Eres Gamer? Pues aqui tienes un articulo para jugar a los juegos que te gustan gracias a este mouse Gamer.',
      category: 'Electrodomestico',
>>>>>>> 06d5b64c22d797e0feb4154bac8bf7fb84047750
      stock: 14,
      img: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRM-M-9y066dAocZ6rOuSprJkGMnOT1MkO-4Q&s'
  },
  {
    id: 2,
    name: 'Teclado inamabrico Rbg e inalambrico',
    price: 53000,
    description: 'Para ser buen Gamer hay que tener un Buen teclado y que mejor que tener uno inalambrico como este',
<<<<<<< HEAD
    category: 'Accesorio para PC',
=======
    category: 'Electrodomestico',
>>>>>>> 06d5b64c22d797e0feb4154bac8bf7fb84047750
    stock: 23,
    img: 'https://i.blogs.es/351af9/41tysnbsxzl._sl500_/original.webp'
},
{
  id: 3,
  name: 'Laptop',
  price: 650000,
  description: 'Laptop HP (15-fc0005la) Una laptop para tipo de trabajo y que mejor que portatil para que tengas todo listo el trabajo que tengas',
  category: 'Computadora portatil',
  stock: 10,
  img: 'https://ssl-product-images.www8-hp.com/digmedialib/prodimg/lowres/c08484411.png'
},
{
  id: 4,
<<<<<<< HEAD
  name: 'Mouse Inalambricos',
  price: 23000,
  description: 'Un mouse Inalambrico y sin colores para que no te moleste en el trabajo o en tus rutinas',
  category: 'Accesorio para PC',
  stock: 17,
  img: 'data:image/jpeg;base64,'
},
{
  id: 5,
  name: 'Micrófono Razer Seiren Mini USB Streaming',
  price: 24990,
  description: 'Patrón de captación supercardioide: El Razer Seiren Mini está sintonizado con un ángulo de recogida más estrecho, por lo que puede centrarse en tu voz mientras garantiza que los ruidos de fondo como escribir y hacer clic no se escuchen. Calidad de grabación profesional: Con su cápsula condensadora de 0.551 pulgadas y respuesta de frecuencia plana, el micrófono transmite tu voz con claridad estelar que está llena de agudos nítidos y bajos profundos.',
  category: 'Accesorio para PC',
  stock: 11,
  img: 'https://imagedelivery.net/4fYuQyy-r8_rpBpcY7lH_A/falabellaCL/113174163_01/w=1500,h=1500,fit=pad'
},
{
  id: 6,
  name: 'Microondas 20 Lt MS20A3010AL Samsung',
  price: 69990,
  description: 'El Microondas Análogo 20 Litros Negro QN32LS03CBGXZS de Samsung es la solución perfecta para tu cocina. Con su diseño moderno en color negro y borde metálico, este microondas se integra a la perfección en cualquier espacio',
  category: 'Electrodomestico',
  stock: 16,
  img: 'https://imagedelivery.net/4fYuQyy-r8_rpBpcY7lH_A/falabellaCL/17187389_1/w=1500,h=1500,fit=pad'
},

{
  id: 7,
  name: 'Cafetera Espresso Oster Primalatte Negra',
  price: 139990,
  description: 'Prepare fácilmente variedades de café aromáticas, como un espresso, café y cappuccino, con solo tocar un botón. LatteGo agrega una cremosa capa de espuma de leche, es fácil de configurar y se puede limpiar en 15 segundos*',
  category: 'Electrodomestico',
  stock: 25,
  img: 'https://imagedelivery.net/4fYuQyy-r8_rpBpcY7lH_A/falabellaCL/14583459_1/w=1500,h=1500,fit=pad'
},
{
  id: 8,
  name: 'Notebook HP 15-eg2503la Intel Core i5 16GB RAM 512GB SSD',
  price: 599990,
  description: 'Este Notebook es bueno para temas de trabajo y igual para jugar , tiene una memoria de almacenamiento 512GB y de Ram 16GB tambien tiene un procesador Intel Core i5-1235U , su sistema operativo windows 11',
  category: 'Computadora portatil',
  stock: 19,
  img: 'https://imagedelivery.net/4fYuQyy-r8_rpBpcY7lH_A/falabellaCL/133958788_01/w=1500,h=1500,fit=pad'
},
{
  id: 9,
  name: 'Tostador FDV Diamond 2R',
  price: 19990,
  description: 'Tostador Diamond Black. Su diseño tipo mosaico en color negro con terminaciones en acero inoxidable completará tu encimera. Capacidad para 2 rebanadas; función cancelar, con la que podrás retirar el pan para un tostado suave; función recalentar y apagado automático',
  category: 'Electrodomestico',
  stock: 19,
  img: 'https://imagedelivery.net/4fYuQyy-r8_rpBpcY7lH_A/falabellaCL/116212578_01/w=1500,h=1500,fit=pad'
},
{
  id: 10,
  name: 'Parrilla Eléctrica 1500 W Recco RP-TG01',
  price: 24990,
  description: 'Descubre la nueva Parrilla Eléctrica 1500 W Recco RP-TG01, una herramienta versátil que cambiará tu forma de cocinar y te permitirá disfrutar de deliciosas comidas al aire libre sin importar el clima. La parrilla eléctrica de la reconocida marca RECCO se destaca por su potente motor de 1500 W que garantiza un rápido precalentamiento y una cocción uniforme, además, cuenta con 5 niveles de temperatura para que puedas ajustarla según tus necesidades.',
  category: 'Electrodomestico',
  stock: 19,
  img: 'https://imagedelivery.net/4fYuQyy-r8_rpBpcY7lH_A/falabellaCL/880789918_1/w=1500,h=1500,fit=pad'
},
{
  id: 11,
  name: 'Notebook Gamer Aspire 5 Laptop A515-58GM-56XX-1 15.6" Intel Core I5 32GB Ram 512GB SSD Acer RTX 2050',
  price: 669990,
  description: 'Siente la potencia que emana desde el interior gracias al procesador Intel Core i5 con 8 Núcleos. La última tecnología en velocidad para una nueva experiencia gamer sin interrupciones ni lag en los juegos más populares. Pisa el acelerador y lleva los gráficos al extremo con la tarjeta de video NVIDIA GeForce RTX 2050 para alcanzar el máximo rendimiento y poder que necesitas para avanzar a la siguiente etapa.',
  category: 'Computadora portatil',
  stock: 10,
  img: 'https://imagedelivery.net/4fYuQyy-r8_rpBpcY7lH_A/falabellaCL/17070604_1/w=1500,h=1500,fit=pad'
},
{
  id: 12,
  name: 'Notebook Vivobook 14 X1404 Intel Core i5 8GB RAM 512GB SSD 14" FHD 60Hz Asus',
  price: 419990,
  description: 'El diseño compacto y liviano de la Vivobook 14 permite llevarla a cualquier lugar con un esfuerzo mínimo, gracias a un peso de apenas 1,4Kg y un grosor de apenas 1,79cm. La Vivobook 14 admite carga rápida, por lo que puedes cargar una batería totalmente descargada al 60% en tan solo 49 minutos. ¡Estará en funcionamiento más rápido que nunca!Tu comodidad es nuestra máxima prioridad. Es por eso que el teclado ASUS ErgoSense tiene una sensación increíblemente satisfactoria, con la profundidad y el recorrido óptimos de las teclas calculados con máxima precisión. Experimenta el teclado ErgoSense y pronto te encontrarás navegando sin esfuerzo a través de tu trabajo.',
  category: 'Computadora portatil',
  stock: 10,
  img: 'https://imagedelivery.net/4fYuQyy-r8_rpBpcY7lH_A/falabellaCL/16874581_01/w=1500,h=1500,fit=pad'
},


=======
  name: 'Mouse',
  price: 23000,
  description: 'Un mouse Inalambrico y sin colores para que no te moleste en el trabajo o en tus rutinas',
  category: 'Mouse inalambrico',
  stock: 17,
  img: 'data:image/jpeg;base64,'
},
>>>>>>> 06d5b64c22d797e0feb4154bac8bf7fb84047750


]


export const getProducts = () =>{
  return new Promise((res) =>{
      setTimeout(() =>{
          res(products);
      }, 2000);
  })
}
